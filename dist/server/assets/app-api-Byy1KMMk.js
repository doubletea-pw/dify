//#region i18n/ja-JP/app-api.json
var apiKey = "API キー";
var apiServer = "API サーバー";
var copied = "コピー済み";
var copy = "コピー";
var disabled = "無効";
var loading = "読み込み中";
...
var app_api_default = {
	...
	"chatMode.conversationIdTip": "（オプション）会話 ID：初回の会話の場合は空白のままにしておき、継続する場合はコンテキストから conversation_id を渡します。",
	...
	"chatMode.inputsTips": "（オプション）Prompt Eng の変数に対応するキーと値のペアとしてユーザー入力フィールドを提供します。キーは変数名で、値はパラメータの値です。フィールドのタイプが Select の場合、送信される値は事前に設定された選択肢のいずれかである必要があります。",
	...
	"chatMode.streaming": "ストリーミングの返却。SSE（Server-Sent Events）に基づいたストリーミングの返却の実装。",
	...
	"completionMode.inputsTips": "（オプション）Prompt Eng の変数に対応するキーと値のペアとしてユーザー入力フィールドを提供します。キーは変数名で、値はパラメータの値です。フィールドのタイプが Select の場合、送信される値は事前に設定された選択肢のいずれかである必要があります。",
	...
	"completionMode.streaming": "ストリーミングの返却。SSE（Server-Sent Events）に基づいたストリーミングの返却の実装。",
