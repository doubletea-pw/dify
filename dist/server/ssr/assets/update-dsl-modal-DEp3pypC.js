import { S as D, j as FC, xt as P7 } from "./react-D5xHzNYo.js";
...
import { Qt as WORKFLOW_DATA_UPDATE, n as initialNodes, t as initialEdges } from "./utils-DqpfZ3W-.js";
import "./declarations-DYMEe7hD.js";
import { Lt as SupportUploadFileTypes, Ot as BlockEnum } from "./config-Dopncj5R.js";
...
import { l as fetchWorkflowDraft } from "./workflow-Dbk_had1.js";
...
//#endregion
//#region app/components/workflow/update-dsl-modal.helpers.ts
var getInvalidNodeTypes = (mode) => {
	if (mode === AppModeEnum.ADVANCED_CHAT) return [
		BlockEnum.End,
...
var validateDSLContent = (content, mode) => {
	try {
		const nodes = load(content)?.workflow?.graph?.nodes ?? [];
...
var isImportCompleted = (status) => {
	return status === DSLImportStatus.COMPLETED || status === DSLImportStatus.COMPLETED_WITH_WARNINGS;
};
var getImportNotificationPayload = (status, t) => {
	return {
		type: status === DSLImportStatus.COMPLETED ? "success" : "warning",
		message: t(status === DSLImportStatus.COMPLETED ? "common.importSuccess" : "common.importWarning", { ns: "workflow" }),
		children: status === DSLImportStatus.COMPLETED_WITH_WARNINGS ? t("common.importWarningDetails", { ns: "workflow" }) : void 0
	};
};
var normalizeWorkflowFeatures = (features) => {
	return {
		file: {
			image: {
...
//#endregion
//#region app/components/workflow/update-dsl-modal.tsx
var UpdateDSLModal = ({ onCancel, onBackup, onImport }) => {
	...
	const handleWorkflowUpdate = useCallback(async (app_id) => {
		const { graph, features, hash, conversation_variables, environment_variables } = await fetchWorkflowDraft(`/apps/${app_id}/workflows/draft`);
	...
	const handleCompletedImport = useCallback(async (status, appId) => {
		if (!appId) {
		...
		toast[payload.type](payload.message, payload.children ? { description: payload.children } : void 0);
	...
	const handlePendingImport = useCallback((id, importedVersion, currentVersion) => {
		setShow(false);
		setTimeout(() => {
	...
	const handleImport = useCallback(async () => {
		...
		try {
			if (appDetail && fileContent && validateDSLContent(fileContent, appDetail.mode)) {
				setLoading(true);
				const { id, status, app_id, imported_dsl_version, current_dsl_version } = await importDSL({
					mode: DSLImportMode.YAML_CONTENT,
					yaml_content: fileContent,
					app_id: appDetail.id
				});
				if (isImportCompleted(status)) await handleCompletedImport(status, app_id);
				else if (status === DSLImportStatus.PENDING) handlePendingImport(id, imported_dsl_version, current_dsl_version);
				else {
					setLoading(false);
					toast.error(t("common.importFailure", { ns: "workflow" }));
				}
			} else if (fileContent) toast.error(t("common.importFailure", { ns: "workflow" }));
		} catch (e) {
			setLoading(false);
			toast.error(t("common.importFailure", { ns: "workflow" }));
		}
	...
	const onUpdateDSLConfirm = async () => {
		try {
			if (!importId) return;
			const { status, app_id } = await importDSLConfirm({ import_id: importId });
			if (isImportCompleted(status)) await handleCompletedImport(status, app_id);
		...
		children: [
			/* @__PURE__ */ jsxs("div", {
				...
				children: [/* @__PURE__ */ jsx("div", {
					...
					children: t("common.importDSL", { ns: "workflow" })
				}), /* @__PURE__ */ jsx("div", {
			...
			/* @__PURE__ */ jsxs("div", {
				className: "relative mb-2 flex grow gap-0.5 overflow-hidden rounded-xl border-[0.5px] border-components-panel-border bg-components-panel-bg-blur p-2 shadow-xs",
				children: [
					/* @__PURE__ */ jsx("div", { className: "absolute left-0 top-0 h-full w-full bg-toast-warning-bg opacity-40" }),
					/* @__PURE__ */ jsx("div", {
						className: "flex items-start justify-center p-1",
						children: /* @__PURE__ */ jsx(D, { className: "h-4 w-4 shrink-0 text-text-warning-secondary" })
					}),
					/* @__PURE__ */ jsxs("div", {
						className: "flex grow flex-col items-start gap-0.5 py-1",
						children: [/* @__PURE__ */ jsx("div", {
							className: "whitespace-pre-line text-text-primary system-xs-medium",
							children: t("common.importDSLTip", { ns: "workflow" })
						}), /* @__PURE__ */ jsx("div", {
							className: "flex items-start gap-1 self-stretch pb-0.5 pt-1",
							children: /* @__PURE__ */ jsxs(Button, {
								size: "small",
								variant: "secondary",
								className: "z-[1000]",
								onClick: onBackup,
								children: [/* @__PURE__ */ jsx(FC, { className: "h-3.5 w-3.5 text-components-button-secondary-text" }), /* @__PURE__ */ jsx("div", {
									className: "flex items-center justify-center gap-1 px-[3px]",
									children: t("common.backupCurrentDraft", { ns: "workflow" })
								})]
							})
						})]
					})
			...
			/* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("div", {
				className: "pt-2 text-text-primary system-md-semibold",
				children: t("common.chooseDSL", { ns: "workflow" })
			}), /* @__PURE__ */ jsx("div", {
				className: "flex w-full flex-col items-start justify-center gap-4 self-stretch py-4",
				children: /* @__PURE__ */ jsx(uploader_default, {
					file: currentFile,
			...
			/* @__PURE__ */ jsxs("div", {
				...
				children: [/* @__PURE__ */ jsx(Button, {
					...
					loading,
					children: t("common.overwriteAndImport", { ns: "workflow" })
				})]
			})
		]
	}), /* @__PURE__ */ jsxs(Modal, {
...
var update_dsl_modal_default = memo(UpdateDSLModal);
//#endregion
export { update_dsl_modal_default as default };
