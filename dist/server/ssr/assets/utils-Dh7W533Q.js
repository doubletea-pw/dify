...
import { Pt as FileAppearanceTypeEnum } from "./hooks-Bj_4KA1b.js";
...
//#endregion
//#region app/components/base/file-uploader/utils.ts
/**
...
*/
var getFileUploadErrorMessage = (error, defaultMessage, t) => {
...
var fileUpload = ({ file, onProgressCallback, onSuccessCallback, onErrorCallback }, isPublic, url) => {
...
var additionalExtensionMap = new Map([["text/x-markdown", ["md"]]]);
var getFileExtension = (fileName, fileMimetype, isRemote) => {
	let extension = "";
	let extensions = /* @__PURE__ */ new Set();
	if (fileMimetype) {
		const extensionsFromMimeType = src_default.getAllExtensions(fileMimetype) || /* @__PURE__ */ new Set();
		const additionalExtensions = additionalExtensionMap.get(fileMimetype) || [];
		extensions = new Set([...extensionsFromMimeType, ...additionalExtensions]);
	}
	...
	if (fileName) {
		...
		if (fileNamePairLength > 1) {
			...
			if (extensions.has(extensionInFileName)) extension = extensionInFileName;
		}
	}
	if (!extension) if (extensions.size > 0) {
		const firstExtension = extensions.values().next().value;
		extension = firstExtension ? firstExtension.toLowerCase() : "";
	} else extension = extensionInFileName;
	if (isRemote) extension = "";
	return extension;
};
var getFileAppearanceType = (fileName, fileMimetype) => {
...
var getSupportFileType = (fileName, fileMimetype, isCustom) => {
	if (isCustom) return SupportUploadFileTypes.custom;
	const extension = getFileExtension(fileName, fileMimetype);
	for (const key in FILE_EXTS) if (FILE_EXTS[key].includes(extension.toUpperCase())) return key;
	return "";
};
var getProcessedFiles = (files) => {
...
var getProcessedFilesFromResponse = (files) => {
	return files.map((fileItem) => {
		...
		if (fileItem.filename && fileItem.mime_type) {
			...
			if (detectedTypeFromFileName && detectedTypeFromMime && detectedTypeFromFileName === detectedTypeFromMime && detectedTypeFromFileName !== fileItem.type) supportFileType = detectedTypeFromFileName;
		}
		return {
...
var getSupportFileExtensionList = (allowFileTypes, allowFileExtensions) => {
...
var isAllowedFileExtension = (fileName, fileMimetype, allowFileTypes, allowFileExtensions) => {
...
var getFilesInLogs = (rawData) => {
	return Object.keys(rawData || {}).map((key) => {
		if (typeof rawData[key] === "object" && rawData[key]?.dify_model_identity === "__dify__file__") return {
			varName: key,
			list: getProcessedFilesFromResponse([rawData[key]])
		};
		if (Array.isArray(rawData[key]) && rawData[key].some((item) => item?.dify_model_identity === "__dify__file__")) return {
			varName: key,
			list: getProcessedFilesFromResponse(rawData[key])
		};
	}).filter(Boolean);
...
//#endregion
export { getFileUploadErrorMessage as a, getProcessedFilesFromResponse as c, getFileExtension as i, getSupportFileType as l, fileUpload as n, getFilesInLogs as o, getFileAppearanceType as r, getProcessedFiles as s, fileIsUploaded as t, isAllowedFileExtension as u };
