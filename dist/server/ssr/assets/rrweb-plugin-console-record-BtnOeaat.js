//#region node_modules/.pnpm/@amplitude+rrweb-plugin-console-record@2.0.0-alpha.36_@amplitude+rrweb@2.0.0-alpha.36/node_modules/@amplitude/rrweb-plugin-console-record/dist/rrweb-plugin-console-record.js
...
function iterateResolveTree(tree, cb) {
	cb(tree.value);
	for (let i2 = tree.children.length - 1; i2 >= 0; i2--) iterateResolveTree(tree.children[i2], cb);
}
...
function getBaseDimension(node2, rootIframe) {
	...
	if (!frameElement || frameElement === rootIframe) return {
		x: 0,
		y: 0,
		relativeScale: 1,
		absoluteScale: 1
...
function getNestedRule(rules2, position) {
	...
	if (position.length === 1) return rule2;
	else return getNestedRule(rule2.cssRules[position[1]].cssRules, position.slice(2));
}
...
var ErrorStackParser = {
	...
	parseOpera9: function(e) {
		...
		for (let i = 2, len = lines.length; i < len; i += 2) {
			...
			if (match) result2.push(new StackFrame({
				fileName: match[2],
...
function pathToSelector(node2) {
	if (!node2 || !node2.outerHTML) return "";
	let path = "";
	while (node2.parentElement) {
		...
		if (domSiblings.length > 1) name += `:eq(${domSiblings.indexOf(node2)})`;
		path = name + (path ? ">" + path : "");
		node2 = parent;
	}
	return path;
...
function stringify(obj, stringifyOptions) {
	...
	return JSON.stringify(obj, function(key, value) {
		if (stack.length > 0) {
			...
			if (~stack.indexOf(value)) if (stack[0] === value) value = "[Circular ~]";
		...
		if (value instanceof Event) {
			...
			for (const eventKey in value) {
				...
				if (Array.isArray(eventValue)) eventResult[eventKey] = pathToSelector(eventValue.length ? eventValue[0] : null);
