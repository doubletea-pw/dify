//#region node_modules/.pnpm/react-pdf-highlighter@8.0.0-rc.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-pdf-highlighter/dist/node_modules/pdfjs-dist/web/pdf_viewer.js
...
var ProgressBar = class {
	...
	setWidth(viewer) {
		...
		if (scrollbarWidth > 0) __privateGet(this, _style).setProperty("--progressBar-end-offset", `${scrollbarWidth}px`);
	}
	setDisableAutoFetch(delay = 5e3) {
		if (isNaN(__privateGet(this, _percent))) return;
		if (__privateGet(this, _disableAutoFetchTimeout)) clearTimeout(__privateGet(this, _disableAutoFetchTimeout));
		this.show();
	...
	hide() {
		if (!__privateGet(this, _visible)) return;
...
var DIACRITICS_EXCEPTION = /* @__PURE__ */ new Set([
	12441,
	12442,
	2381,
	2509,
...
var DIACRITICS_REG_EXP = /* @__PURE__ */ new RegExp("\\p{M}+", "gu");
var SPECIAL_CHARS_REG_EXP = /* @__PURE__ */ new RegExp("([.*+?^${}()|[\\]\\\\])|(\\p{P})|(\\s+)|(\\p{M})|(\\p{L})", "gu");
var NOT_DIACRITIC_FROM_END_REG_EXP = /* @__PURE__ */ new RegExp("([^\\p{M}])\\p{M}*$", "u");
var NOT_DIACRITIC_FROM_START_REG_EXP = /* @__PURE__ */ new RegExp("^\\p{M}*([^\\p{M}])", "u");
...
function normalize(text) {
	...
	normalized = normalized.replace(normalizationRegex, (match2, p1, p2, p3, p4, p5, p6, p7, p8, i) => {
		...
		if (p3) {
			hasDiacritics = true;
			if (i + eol === ((_a = rawDiacriticsPositions[rawDiacriticsIndex]) == null ? void 0 : _a[1])) ++rawDiacriticsIndex;
			else {
...
function getOriginalIndex(diffs, pos, len) {
	...
	return [oldStart, end + diffs[j][1] + 1 - oldStart];
}
var PDFFindController = class {
	...
	setDocument(pdfDocument) {
		if (this._pdfDocument) __privateMethod(this, _PDFFindController_instances, reset_fn).call(this);
...
onFind_fn = function(state) {
	...
	const { type } = state;
	if (__privateGet(this, _state) === null || __privateMethod(this, _PDFFindController_instances, shouldDirtyMatch_fn).call(this, state)) this._dirtyMatch = true;
	__privateSet(this, _state, state);
	if (type !== "highlightallchange") __privateMethod(this, _PDFFindController_instances, updateUIState_fn).call(this, FindState.PENDING);
	this._firstPageCapability.promise.then(() => {
		...
		else if (this._dirtyMatch) __privateMethod(this, _PDFFindController_instances, nextMatch_fn).call(this);
		else if (type === "again") {
			__privateMethod(this, _PDFFindController_instances, nextMatch_fn).call(this);
			if (findbarClosed && __privateGet(this, _state).highlightAll) __privateMethod(this, _PDFFindController_instances, updateAllPages_fn).call(this);
		} else if (type === "highlightallchange") {
			if (pendingTimeout) __privateMethod(this, _PDFFindController_instances, nextMatch_fn).call(this);
			else this._highlightMatches = true;
			__privateMethod(this, _PDFFindController_instances, updateAllPages_fn).call(this);
		} else __privateMethod(this, _PDFFindController_instances, nextMatch_fn).call(this);
	});
};
reset_fn = function() {
...
query_get = function() {
	const { query } = __privateGet(this, _state);
...
calculateRegExpMatch_fn = function(query, entireWord, pageIndex, pageContent) {
	...
	while ((match2 = query.exec(pageContent)) !== null) {
		if (entireWord && !__privateMethod(this, _PDFFindController_instances, isEntireWord_fn).call(this, pageContent, match2.index, match2[0].length)) continue;
...
convertToRegExpString_fn = function(query, hasDiacritics) {
	const { matchDiacritics } = __privateGet(this, _state);
	let isUnicode = false;
...
calculateMatch_fn = function(pageIndex) {
	...
	const { caseSensitive, entireWord } = __privateGet(this, _state);
	const pageContent = this._pageContents[pageIndex];
	const hasDiacritics = this._hasDiacritics[pageIndex];
	let isUnicode = false;
	if (typeof query === "string") [isUnicode, query] = __privateMethod(this, _PDFFindController_instances, convertToRegExpString_fn).call(this, query, hasDiacritics);
	...
	query = query ? new RegExp(query, `g${isUnicode ? "u" : ""}${caseSensitive ? "" : "i"}`) : null;
	...
	if (__privateGet(this, _state).highlightAll) __privateMethod(this, _PDFFindController_instances, updatePage_fn).call(this, pageIndex);
	...
	if (__privateGet(this, _updateMatchesCountOnProgress)) {
		if (pageMatchesCount > 0) __privateMethod(this, _PDFFindController_instances, updateUIResultsCount_fn).call(this);
	} else if (++__privateWrapper(this, _visitedPagesCount)._ === this._linkService.pagesCount) __privateMethod(this, _PDFFindController_instances, updateUIResultsCount_fn).call(this);
};
...
nextMatch_fn = function() {
	...
	if (__privateGet(this, _PDFFindController_instances, query_get).length === 0) {
...
updateMatch_fn = function(found = false) {
	...
	if (found) {
		...
		if (previousPage !== -1 && previousPage !== this._selected.pageIdx) __privateMethod(this, _PDFFindController_instances, updatePage_fn).call(this, previousPage);
	}
...
requestMatchesCount_fn = function() {
	...
	if (matchIdx !== -1) {
		for (let i = 0; i < pageIdx; i++) current += ((_a = this._pageMatches[i]) == null ? void 0 : _a.length) || 0;
		current += matchIdx + 1;
	}
...
updateUIState_fn = function(state, previous = false) {
	var _a, _b;
	if (!__privateGet(this, _updateMatchesCountOnProgress) && (__privateGet(this, _visitedPagesCount) !== this._linkService.pagesCount || state === FindState.PENDING)) return;
...
var _PDFLinkService = class _PDFLinkService {
	...
	executeNamedAction(action) {
		...
		switch (action) {
			case "GoBack":
				(_a = this.pdfHistory) == null || _a.back();
				break;
			case "GoForward":
				(_b = this.pdfHistory) == null || _b.forward();
				break;
			case "NextPage":
...
isValidExplicitDest_fn = function(dest) {
	...
	if (!(typeof page === "object" && Number.isInteger(page == null ? void 0 : page.num) && Number.isInteger(page == null ? void 0 : page.gen)) && !Number.isInteger(page)) return false;
	if (!(typeof zoom === "object" && typeof (zoom == null ? void 0 : zoom.name) === "string")) return false;
	let allowNull = true;
	switch (zoom.name) {
		case "XYZ":
			...
			break;
		case "Fit":
		case "FitB": return args.length === 0;
		case "FitH":
		case "FitBH":
		case "FitV":
		case "FitBV":
...
var AnnotationLayerBuilder = class {
	...
	async render(viewport, intent = "display") {
		...
		(_a = __privateGet(this, _onAppend)) == null || _a.call(this, div);
		...
		if (this.linkService.isInPresentationMode) __privateMethod(this, _AnnotationLayerBuilder_instances, updatePresentationModeState_fn).call(this, PresentationModeState.FULLSCREEN);
		if (!__privateGet(this, _eventAbortController)) {
			__privateSet(this, _eventAbortController, new AbortController());
			(_b = this._eventBus) == null || _b._on("presentationmodechanged", (evt) => {
	...
	cancel() {
		...
		(_a = __privateGet(this, _eventAbortController)) == null || _a.abort();
...
var EventBus = class {
	...
	_on(eventName, listener, options = null) {
		...
		((_a = __privateGet(this, _listeners))[eventName] || (_a[eventName] = [])).push({
			listener,
	...
	_off(eventName, listener, options = null) {
		...
		for (let i = 0, ii = eventListeners.length; i < ii; i++) {
			...
			if (evt.listener === listener) {
				(_a = evt.rmAbort) == null || _a.call(evt);
...
var Scope = class {
	...
	memoizeIntlObject(ctor, opts) {
		...
		if (!cache2[id]) cache2[id] = new ctor(this.bundle.locales, opts);
		return cache2[id];
	}
};
...
var _L10n = class _L10n {
	...
	async get(ids, args = null, fallback) {
		if (Array.isArray(ids)) {
			ids = ids.map((id) => ({ id }));
			return (await __privateGet(this, _l10n).formatMessages(ids)).map((message) => message.value);
		}
	...
	async destroy() {
		for (const element of __privateGet(this, _elements)) __privateGet(this, _l10n).disconnectRoot(element);
...
generateBundles_fn = async function* (defaultLang, baseLang) {
	const { baseURL, paths } = await __privateMethod(this, _genericl10n_GenericL10n_static, getPaths_fn).call(this);
	...
	for (const lang of langs) {
		...
		else if (lang === "en-us") yield __privateMethod(this, _genericl10n_GenericL10n_static, createBundleFallback_fn).call(this, lang);
	}
};
...
var PDFHistory = class {
	...
	initialize({ fingerprint, resetHistory = false, updateUrl = false }) {
		...
		if (!__privateMethod(this, _PDFHistory_instances, isValidState_fn).call(this, state, true) || resetHistory) {
			const { hash, page, rotation } = __privateMethod(this, _PDFHistory_instances, parseCurrentHash_fn).call(this, true);
	...
	pushPage(pageNumber) {
		...
		if (((_a = this._destination) == null ? void 0 : _a.page) === pageNumber) return;
	...
	back() {
		if (!this._initialized || this._popStateInProgress) return;
		const state = window.history.state;
		if (__privateMethod(this, _PDFHistory_instances, isValidState_fn).call(this, state) && state.uid > 0) window.history.back();
	}
	forward() {
		if (!this._initialized || this._popStateInProgress) return;
		const state = window.history.state;
		if (__privateMethod(this, _PDFHistory_instances, isValidState_fn).call(this, state) && state.uid < this._maxUid) window.history.forward();
	}
...
isValidState_fn = function(state, checkReload = false) {
	...
	if (state.fingerprint !== this._fingerprint) if (checkReload) {
		...
		if ((perfEntry == null ? void 0 : perfEntry.type) !== "reload") return false;
	} else return false;
...
updateInternalState_fn = function(destination, uid, removeTemporary = false) {
	...
	if (removeTemporary && (destination == null ? void 0 : destination.temporary)) delete destination.temporary;
...
parseCurrentHash_fn = function(checkNameddest = false) {
	...
	if (!__privateMethod(this, _PDFHistory_instances, isValidPage_fn).call(this, page) || checkNameddest && nameddest.length > 0) page = null;
	return {
		hash,
		page,
		rotation: this.linkService.rotation
...
updateViewarea_fn = function({ location }) {
	if (this._updateViewareaTimeout) {
		clearTimeout(this._updateViewareaTimeout);
		this._updateViewareaTimeout = null;
	}
	...
	this._updateViewareaTimeout = setTimeout(() => {
		if (!this._popStateInProgress) __privateMethod(this, _PDFHistory_instances, tryPushCurrentPosition_fn).call(this, true);
		this._updateViewareaTimeout = null;
	}, UPDATE_VIEWAREA_TIMEOUT);
};
popState_fn = function({ state }) {
	const newHash = getCurrentHash(), hashChanged = this._currentHash !== newHash;
	this._currentHash = newHash;
	if (!state) {
		this._uid++;
		const { hash, page, rotation } = __privateMethod(this, _PDFHistory_instances, parseCurrentHash_fn).call(this);
		__privateMethod(this, _PDFHistory_instances, pushOrReplaceState_fn).call(this, {
			hash,
			page,
			rotation
	...
	if (!__privateMethod(this, _PDFHistory_instances, isValidState_fn).call(this, state)) return;
	this._popStateInProgress = true;
	if (hashChanged) {
...
pageHide_fn = function() {
	if (!this._destination || this._destination.temporary) __privateMethod(this, _PDFHistory_instances, tryPushCurrentPosition_fn).call(this);
};
bindEvents_fn = function() {
	if (__privateGet(this, _eventAbortController2)) return;
	__privateSet(this, _eventAbortController2, new AbortController());
	const { signal } = __privateGet(this, _eventAbortController2);
	this.eventBus._on("updateviewarea", __privateMethod(this, _PDFHistory_instances, updateViewarea_fn).bind(this), { signal });
	window.addEventListener("popstate", __privateMethod(this, _PDFHistory_instances, popState_fn).bind(this), { signal });
	window.addEventListener("pagehide", __privateMethod(this, _PDFHistory_instances, pageHide_fn).bind(this), { signal });
};
unbindEvents_fn = function() {
	var _a;
	(_a = __privateGet(this, _eventAbortController2)) == null || _a.abort();
...
var AnnotationEditorLayerBuilder = class {
	...
	async render(viewport, intent = "display") {
		...
		(_a = __privateGet(this, _onAppend2)) == null || _a.call(this, div);
...
_uiManager = /* @__PURE__ */ new WeakMap();
{
	...
	(function checkCanvasSizeLimitation() {
		if (isIOS || isAndroid) compatibilityParams.maxCanvasPixels = 5242880;
	})();
}
var OptionKind = {
	BROWSER: 1,
...
for (const name in compatibilityParams) userOptions[name] = compatibilityParams[name];
var AppOptions = class {
	...
	static getAll(kind = null, defaultOnly = false) {
		...
		for (const name in defaultOptions) {
			...
			if (kind && !(kind & defaultOption.kind)) continue;
	...
	static setAll(options, init = false) {
		if (init) {
			...
			for (const name in userOptions) {
				if (compatibilityParams[name] !== void 0) continue;
		...
		for (const name in options) userOptions[name] = options[name];
	}
	static remove(name) {
		delete userOptions[name];
		const val = compatibilityParams[name];
		if (val !== void 0) userOptions[name] = val;
	}
};
var DrawLayerBuilder = class {
	...
	async render(intent = "display") {
		if (intent !== "display" || __privateGet(this, _drawLayer2) || this._cancelled) return;
		__privateSet(this, _drawLayer2, new DrawLayer({ pageIndex: this.pageIndex }));
	}
	cancel() {
		this._cancelled = true;
		if (!__privateGet(this, _drawLayer2)) return;
		__privateGet(this, _drawLayer2).destroy();
		__privateSet(this, _drawLayer2, null);
	}
	setParent(parent) {
		var _a;
		(_a = __privateGet(this, _drawLayer2)) == null || _a.setParent(parent);
	}
	getDrawLayer() {
...
var StructTreeLayerBuilder = class {
	...
	render(structTree) {
		if (__privateGet(this, _treeDom) !== void 0) return __privateGet(this, _treeDom);
	...
	hide() {
		if (__privateGet(this, _treeDom) && !__privateGet(this, _treeDom).hidden) __privateGet(this, _treeDom).hidden = true;
	}
	show() {
		var _a;
		if ((_a = __privateGet(this, _treeDom)) == null ? void 0 : _a.hidden) __privateGet(this, _treeDom).hidden = false;
	}
};
...
setAttributes_fn = function(structElement, htmlElement) {
	...
	if (alt !== void 0) htmlElement.setAttribute("aria-label", removeNullCharacters(alt));
...
walk_fn = function(node) {
	...
	if (node.children) if (node.children.length === 1 && "id" in node.children[0]) __privateMethod(this, _StructTreeLayerBuilder_instances, setAttributes_fn).call(this, node.children[0], element);
	else for (const kid of node.children) element.append(__privateMethod(this, _StructTreeLayerBuilder_instances, walk_fn).call(this, kid));
	return element;
};
var _TextAccessibilityManager = class _TextAccessibilityManager {
	...
	enable() {
		if (__privateGet(this, _enabled)) throw new Error("TextAccessibilityManager is already enabled.");
		if (!__privateGet(this, _textChildren)) throw new Error("Text divs and strings have not been set.");
		...
		for (const [element, isRemovable] of __privateGet(this, _waitingElements)) this.addPointerInTextLayer(element, isRemovable);
...
addIdToAriaOwns_fn = function(id, node) {
	...
	if (!(owns == null ? void 0 : owns.includes(id))) node.setAttribute("aria-owns", owns ? `${owns} ${id}` : id);
...
var TextHighlighter = class {
	...
	disable() {
		...
		this.enabled = false;
		(_a = __privateGet(this, _eventAbortController3)) == null || _a.abort();
	...
	_updateMatches(reset = false) {
		...
		if (!(findController == null ? void 0 : findController.highlightMatches) || reset) return;
...
var _TextLayerBuilder = class _TextLayerBuilder {
	...
	async render(viewport, textContentParams = null) {
		...
		const { textDivs, textContentItemsStr } = __privateGet(this, _textLayer2);
		(_a = this.highlighter) == null || _a.setTextMapping(textDivs, textContentItemsStr);
		(_b = this.accessibilityManager) == null || _b.setTextMapping(textDivs);
		...
		(_c = __privateGet(this, _onAppend3)) == null || _c.call(this, this.div);
		(_d = this.highlighter) == null || _d.enable();
		(_e = this.accessibilityManager) == null || _e.enable();
	}
	hide() {
		var _a;
		if (!this.div.hidden && __privateGet(this, _renderingDone)) {
			(_a = this.highlighter) == null || _a.disable();
			this.div.hidden = true;
		}
	}
	show() {
		...
		if (this.div.hidden && __privateGet(this, _renderingDone)) {
			this.div.hidden = false;
			(_a = this.highlighter) == null || _a.enable();
		}
	}
	cancel() {
		var _a, _b, _c, _d;
		(_a = __privateGet(this, _textLayer2)) == null || _a.cancel();
		__privateSet(this, _textLayer2, null);
		(_b = this.highlighter) == null || _b.disable();
		(_c = this.accessibilityManager) == null || _c.disable();
...
removeGlobalSelectionListener_fn = function(textLayerDiv) {
	...
	if (__privateGet(this, _textLayers).size === 0) {
		(_a = __privateGet(this, _selectionChangeAbortController)) == null || _a.abort();
		__privateSet(this, _selectionChangeAbortController, null);
	}
};
enableGlobalSelectionListener_fn = function() {
	if (__privateGet(this, _selectionChangeAbortController)) return;
	__privateSet(this, _selectionChangeAbortController, new AbortController());
	const { signal } = __privateGet(this, _selectionChangeAbortController);
	...
	document.addEventListener("selectionchange", () => {
		...
		for (let i = 0; i < selection.rangeCount; i++) {
			...
			for (const textLayerDiv of __privateGet(this, _textLayers).keys()) if (!activeTextLayers.has(textLayerDiv) && range2.intersectsNode(textLayerDiv)) activeTextLayers.add(textLayerDiv);
		}
		for (const [textLayerDiv, endDiv2] of __privateGet(this, _textLayers)) if (activeTextLayers.has(textLayerDiv)) endDiv2.classList.add("active");
		else reset(endDiv2, textLayerDiv);
...
var LAYERS_ORDER = /* @__PURE__ */ new Map([
	["canvasWrapper", 0],
	["textLayer", 1],
	["annotationLayer", 2],
...
var PDFPageView = class {
	...
	set renderingState(state) {
		if (state === __privateGet(this, _renderingState)) return;
	...
	setPdfPage(pdfPage) {
		...
		if (this._isStandalone && (((_a = this.pageColors) == null ? void 0 : _a.foreground) === "CanvasText" || ((_b = this.pageColors) == null ? void 0 : _b.background) === "Canvas")) {
			(_c = this._container) == null || _c.style.setProperty("--hcm-highlight-filter", pdfPage.filterFactory.addHighlightHCMFilter("highlight", "CanvasText", "Canvas", "HighlightText", "Highlight"));
			(_d = this._container) == null || _d.style.setProperty("--hcm-highlight-selected-filter", pdfPage.filterFactory.addHighlightHCMFilter("highlight_selected", "CanvasText", "Canvas", "HighlightText", "Highlight"));
		}
		this.pdfPage = pdfPage;
	...
	destroy() {
		var _a;
		this.reset();
		(_a = this.pdfPage) == null || _a.cleanup();
	}
	...
	reset({ keepZoomLayer = false, keepAnnotationLayer = false, keepAnnotationEditorLayer = false, keepXfaLayer = false, keepTextLayer = false } = {}) {
		...
		for (let i = childNodes.length - 1; i >= 0; i--) {
			...
			switch (node) {
				...
				case xfaLayerNode:
				case textLayerNode: continue;
			}
			node.remove();
			const layerIndex = __privateGet(this, _layers).indexOf(node);
			if (layerIndex >= 0) __privateGet(this, _layers)[layerIndex] = null;
		}
		...
		(_e = this.structTreeLayer) == null || _e.hide();
		if (!zoomLayerNode) {
			if (this.canvas) {
	...
	update({ scale = 0, rotation = null, optionalContentConfigPromise = null, drawingDelay = -1 }) {
		...
		__privateMethod(this, _PDFPageView_instances, setDimensions_fn).call(this);
		if (this._isStandalone) (_a = this._container) == null || _a.style.setProperty("--scale-factor", this.viewport.scale);
		if (this.canvas) {
	...
	cancelRendering({ keepAnnotationLayer = false, keepAnnotationEditorLayer = false, keepXfaLayer = false, keepTextLayer = false, cancelExtraDelay = 0 } = {}) {
		...
		if (this.xfaLayer && (!keepXfaLayer || !this.xfaLayer.div)) {
			...
			this.xfaLayer = null;
			(_a = this._textHighlighter) == null || _a.disable();
		}
	}
	cssTransform({ target, redrawAnnotationLayer = false, redrawAnnotationEditorLayer = false, redrawXfaLayer = false, redrawTextLayer = false, hideTextLayer = false }) {
		...
		if (redrawAnnotationLayer && this.annotationLayer) __privateMethod(this, _PDFPageView_instances, renderAnnotationLayer_fn).call(this);
		if (redrawAnnotationEditorLayer && this.annotationEditorLayer) {
			if (this.drawLayer) __privateMethod(this, _PDFPageView_instances, renderDrawLayer_fn).call(this);
			__privateMethod(this, _PDFPageView_instances, renderAnnotationEditorLayer_fn).call(this);
		}
		if (redrawXfaLayer && this.xfaLayer) __privateMethod(this, _PDFPageView_instances, renderXfaLayer_fn).call(this);
		if (this.textLayer) {
			if (hideTextLayer) {
				this.textLayer.hide();
				(_a = this.structTreeLayer) == null || _a.hide();
			} else if (redrawTextLayer) __privateMethod(this, _PDFPageView_instances, renderTextLayer_fn).call(this);
		}
	}
	get width() {
	...
	async draw() {
		...
		if (!this.annotationLayer && __privateGet(this, _annotationMode) !== AnnotationMode.DISABLE) {
			const { annotationStorage, annotationEditorUIManager, downloadManager, enableScripting, fieldObjectsPromise, hasJSActionsPromise, linkService } = __privateGet(this, _layerProperties);
		...
		if (this.maxCanvasPixels === 0) {
			...
			if (outputScale.sx > maxScale || outputScale.sy > maxScale) {
				...
				__privateSet(this, _hasRestrictedScaling, true);
			} else __privateSet(this, _hasRestrictedScaling, false);
		}
		...
		const resultPromise = renderTask.promise.then(async () => {
			...
			if (this.annotationLayer) await __privateMethod(this, _PDFPageView_instances, renderAnnotationLayer_fn).call(this);
			const { annotationEditorUIManager } = __privateGet(this, _layerProperties);
			if (!annotationEditorUIManager) return;
			this.drawLayer || (this.drawLayer = new DrawLayerBuilder({ pageIndex: this.id }));
		...
		if (pdfPage.isPureXfa) {
			if (!this.xfaLayer) {
				const { annotationStorage, linkService } = __privateGet(this, _layerProperties);
	...
	get thumbnailCanvas() {
		const { directDrawing, initialOptionalContent, regularAnnotations } = __privateGet(this, _useThumbnailCanvas);
...
setDimensions_fn = function() {
	const { viewport } = this;
	if (this.pdfPage) {
		if (__privateGet(this, _previousRotation) === viewport.rotation) return;
...
renderXfaLayer_fn = async function() {
	...
	try {
		...
		if ((result == null ? void 0 : result.textDivs) && this._textHighlighter) __privateMethod(this, _PDFPageView_instances, buildXfaTextContentItems_fn).call(this, result.textDivs);
	} catch (ex) {
...
renderStructTreeLayer_fn = async function() {
	...
	if (treeDom) {
		this.l10n.pause();
		(_b = this.canvas) == null || _b.append(treeDom);
		this.l10n.resume();
	}
	(_c = this.structTreeLayer) == null || _c.show();
};
...
var PDFScriptingManager = class {
	constructor({ eventBus, externalServices = null, docProperties: docProperties2 = null }) {
	...
	async setDocument(pdfDocument) {
		var _a;
		if (__privateGet(this, _pdfDocument)) await __privateMethod(this, _PDFScriptingManager_instances, destroyScripting_fn).call(this);
		...
		if (pdfDocument !== __privateGet(this, _pdfDocument)) return;
		try {
		...
		const { signal } = __privateGet(this, _eventAbortController4);
		eventBus._on("updatefromsandbox", (event) => {
			if ((event == null ? void 0 : event.source) === window) __privateMethod(this, _PDFScriptingManager_instances, updateFromSandbox_fn).call(this, event.detail);
		}, { signal });
		eventBus._on("dispatcheventinsandbox", (event) => {
			var _a2;
			(_a2 = __privateGet(this, _scripting)) == null || _a2.dispatchEventInSandbox(event.detail);
		}, { signal });
		...
		eventBus._on("pagerendered", ({ pageNumber }) => {
			...
			if (pageNumber !== __privateGet(this, _pdfViewer).currentPageNumber) return;
			__privateMethod(this, _PDFScriptingManager_instances, dispatchPageOpen_fn).call(this, pageNumber);
		}, { signal });
		eventBus._on("pagesdestroy", async () => {
			...
			(_b = __privateGet(this, _closeCapability)) == null || _b.resolve();
		}, { signal });
		try {
		...
		Promise.resolve().then(() => {
			if (pdfDocument === __privateGet(this, _pdfDocument)) __privateSet(this, _ready, true);
		});
	}
	...
	async dispatchWillPrint() {
		var _a;
		if (!__privateGet(this, _scripting)) return;
...
updateFromSandbox_fn = async function(detail) {
	...
	if (!id) {
		switch (command) {
			...
			case "print":
				...
				__privateGet(this, _eventBus).dispatch("print", { source: this });
				break;
			case "println":
				console.log(value);
				break;
			case "zoom":
				...
				break;
			case "SaveAs":
				__privateGet(this, _eventBus).dispatch("download", { source: this });
				break;
			case "FirstPage":
			...
			case "WillPrintFinished":
				(_a = __privateGet(this, _willPrintCapability)) == null || _a.resolve();
				__privateSet(this, _willPrintCapability, null);
				break;
		}
		return;
	...
	for (const elementId of ids) {
		...
		else (_b = __privateGet(this, _pdfDocument)) == null || _b.annotationStorage.setValue(elementId, detail);
	}
};
dispatchPageOpen_fn = async function(pageNumber, initialize = false) {
	...
	if (initialize) __privateSet(this, _closeCapability, Promise.withResolvers());
	if (!__privateGet(this, _closeCapability)) return;
...
initScripting_fn = function() {
	...
	if (__privateGet(this, _scripting)) throw new Error("#initScripting: Scripting already exists.");
...
destroyScripting_fn = async function() {
	...
	if (!__privateGet(this, _scripting)) {
		...
		(_a = __privateGet(this, _destroyCapability)) == null || _a.resolve();
		return;
	}
	...
	(_b = __privateGet(this, _willPrintCapability)) == null || _b.reject(/* @__PURE__ */ new Error("Scripting destroyed."));
	__privateSet(this, _willPrintCapability, null);
	(_c = __privateGet(this, _eventAbortController4)) == null || _c.abort();
	__privateSet(this, _eventAbortController4, null);
	this._pageOpenPending.clear();
	this._visitedPages.clear();
	__privateSet(this, _scripting, null);
	__privateSet(this, _ready, false);
	(_d = __privateGet(this, _destroyCapability)) == null || _d.resolve();
};
var PDFScriptingManagerComponents = class extends PDFScriptingManager {
	constructor(options) {
		...
		options.externalServices || (options.externalServices = { createScripting: () => new GenericScripting(options.sandboxBundleSrc) });
...
var PDFRenderingQueue = class {
	...
	renderHighestPriority(currentlyVisiblePages) {
		...
		if (this.isThumbnailViewEnabled && ((_a = this.pdfThumbnailViewer) == null ? void 0 : _a.forceRendering())) return;
...
var PDFPageViewBuffer = class {
	...
	push(view) {
		...
		buf.add(view);
		if (buf.size > __privateGet(this, _size)) __privateMethod(this, _PDFPageViewBuffer_instances, destroyFirstView_fn).call(this);
	}
	resize(newSize, idsToKeep = null) {
		...
		while (buf.size > __privateGet(this, _size)) __privateMethod(this, _PDFPageViewBuffer_instances, destroyFirstView_fn).call(this);
	}
	has(view) {
...
var PDFViewer = class {
	constructor(options) {
		...
		if (((_a = this.container) == null ? void 0 : _a.tagName) !== "DIV" || ((_b = this.viewer) == null ? void 0 : _b.tagName) !== "DIV") throw new Error("Invalid `container` and/or `viewer` option.");
		...
		this.eventBus._on("thumbnailrendered", ({ pageNumber, pdfPage }) => {
			...
			if (!__privateGet(this, _buffer).has(pageView)) pdfPage?.cleanup();
		});
	...
	_setCurrentPageNumber(val, resetCurrentPageView = false) {
		...
		if (this._currentPageNumber === val) {
			if (resetCurrentPageView) __privateMethod(this, _PDFViewer_instances, resetCurrentPageView_fn).call(this);
			return true;
		}
	...
	set currentScale(val) {
		...
		if (!this.pdfDocument) return;
		__privateMethod(this, _PDFViewer_instances, setScale_fn).call(this, val, { noScroll: false });
	}
	...
	set pagesRotation(rotation) {
		...
		if (this._currentScaleValue) __privateMethod(this, _PDFViewer_instances, setScale_fn).call(this, this._currentScaleValue, { noScroll: true });
	...
	async getAllText() {
		...
		for (let pageNum = 1, pagesCount = this.pdfDocument.numPages; pageNum <= pagesCount; ++pageNum) {
			if (__privateGet(this, _interruptCopyCondition)) return null;
			buffer.length = 0;
	...
	setDocument(pdfDocument) {
		...
		if (this.pdfDocument) {
			...
			this._resetView();
			(_a = this.findController) == null || _a.setDocument(null);
			(_b = this._scriptingManager) == null || _b.setDocument(null);
		...
		const { signal } = __privateGet(this, _eventAbortController5);
		...
		Promise.all([firstPagePromise, permissionsPromise]).then(([firstPdfPage, permissions]) => {
			...
			this._optionalContentConfigPromise = optionalContentConfigPromise;
			const { annotationEditorMode, annotationMode, textLayerMode } = __privateMethod(this, _PDFViewer_instances, initializePermissions_fn).call(this, permissions);
			...
			if (annotationEditorMode !== AnnotationEditorType.DISABLE) {
				...
				if (pdfDocument.isPureXfa) console.warn("Warning: XFA-editing is not implemented.");
					...
					if (mode !== AnnotationEditorType.NONE) __privateGet(this, _annotationEditorUIManager).updateMode(mode);
			...
			(_a2 = this._pages[0]) == null || _a2.setPdfPage(firstPdfPage);
			if (this._scrollMode === ScrollMode.PAGE) __privateMethod(this, _PDFViewer_instances, ensurePageViewVisible_fn).call(this);
			...
			__privateMethod(this, _PDFViewer_instances, onePageRenderedOrForceFetch_fn).call(this, signal).then(async () => {
				var _a3, _b2;
				if (pdfDocument !== this.pdfDocument) return;
				(_a3 = this.findController) == null || _a3.setDocument(pdfDocument);
				(_b2 = this._scriptingManager) == null || _b2.setDocument(pdfDocument);
				if (__privateGet(this, _hiddenCopyElement)) document.addEventListener("copy", __privateMethod(this, _PDFViewer_instances, copyCallback_fn).bind(this, textLayerMode), { signal });
				if (__privateGet(this, _annotationEditorUIManager)) eventBus.dispatch("annotationeditormodechanged", {
					source: this,
	...
	setPageLabels(labels) {
		...
		for (let i = 0, ii = this._pages.length; i < ii; i++) this._pages[i].setPageLabel(((_a = this._pageLabels) == null ? void 0 : _a[i]) ?? null);
	}
	_resetView() {
		var _a, _b;
		this._pages = [];
		...
		(_a = __privateGet(this, _eventAbortController5)) == null || _a.abort();
		__privateSet(this, _eventAbortController5, null);
		this.viewer.textContent = "";
		this._updateScrollMode();
		this.viewer.removeAttribute("lang");
		(_b = __privateGet(this, _hiddenCopyElement)) == null || _b.remove();
	...
	_updateScrollMode(pageNumber = null) {
		...
		if (scrollMode === ScrollMode.PAGE) __privateMethod(this, _PDFViewer_instances, ensurePageViewVisible_fn).call(this);
		else if (this._previousScrollMode === ScrollMode.PAGE) this._updateSpreadMode();
		if (this._currentScaleValue && isNaN(this._currentScaleValue)) __privateMethod(this, _PDFViewer_instances, setScale_fn).call(this, this._currentScaleValue, { noScroll: true });
		this._setCurrentPageNumber(pageNumber, true);
		this.update();
	}
	get spreadMode() {
		...
		switch (this._scrollMode) {
			case ScrollMode.WRAPPED: {
				...
				for (const { id, y, percent, widthPercent } of views) {
					...
					if (!yArray) pageLayout.set(y, yArray || (yArray = []));
					yArray.push(id);
				}
	...
	set annotationEditorMode({ mode, editId = null, isFromKeyboard = false }) {
		if (!__privateGet(this, _annotationEditorUIManager)) throw new Error(`The AnnotationEditor is not enabled.`);
		if (__privateGet(this, _annotationEditorMode) === mode) return;
...
initializePermissions_fn = function(permissions) {
	...
	if (!permissions.includes(PermissionFlag.COPY) && __privateGet(this, _textLayerMode2) === TextLayerMode.ENABLE) params.textLayerMode = TextLayerMode.ENABLE_PERMISSIONS;
	if (!permissions.includes(PermissionFlag.MODIFY_CONTENTS)) params.annotationEditorMode = AnnotationEditorType.DISABLE;
	if (!permissions.includes(PermissionFlag.MODIFY_ANNOTATIONS) && !permissions.includes(PermissionFlag.FILL_INTERACTIVE_FORMS) && __privateGet(this, _annotationMode2) === AnnotationMode.ENABLE_FORMS) params.annotationMode = AnnotationMode.ENABLE;
	return params;
};
...
copyCallback_fn = function(textLayerMode, event) {
	...
	if (anchorNode && focusNode && selection.containsNode(__privateGet(this, _hiddenCopyElement))) {
		...
		const ac = new AbortController();
		window.addEventListener("keydown", (ev) => __privateSet(this, _interruptCopyCondition, ev.key === "Escape"), { signal: ac.signal });
...
setScaleUpdatePages_fn = function(newScale, newValue, { noScroll = false, preset = false, drawingDelay = -1, origin = null }) {
	...
	if (postponeDrawing) __privateSet(this, _scaleTimeoutId, setTimeout(() => {
		__privateSet(this, _scaleTimeoutId, null);
		this.refresh();
	}, drawingDelay));
...
resetCurrentPageView_fn = function() {
	...
	if (this.isInPresentationMode) __privateMethod(this, _PDFViewer_instances, setScale_fn).call(this, this._currentScaleValue, { noScroll: true });
	__privateMethod(this, _PDFViewer_instances, scrollIntoView_fn).call(this, pageView);
};
...
getScrollAhead_fn = function(visible) {
	var _a, _b;
	if (((_a = visible.first) == null ? void 0 : _a.id) === 1) return true;
	else if (((_b = visible.last) == null ? void 0 : _b.id) === this.pagesCount) return false;
	switch (this._scrollMode) {
		case ScrollMode.PAGE: return __privateGet(this, _scrollModePageState).scrollDown;
